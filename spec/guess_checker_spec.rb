...
describe 'GuessChecker' do
  ...
  it 'returns the number of correct color_and_position (1)' do
    code          = %w(R B G Y)
    guess         = 'BGRY'
    guess_checker = GuessChecker.new(code, guess)
    expect(guess_checker.color_and_position).to eq(1)
  end
  ...
  it 'returns the number of correct color_and_position (3)' do
    code          = %w(R B G Y)
    guess         = 'RBGG'
    guess_checker = GuessChecker.new(code, guess)
    expect(guess_checker.color_and_position).to eq(3)
  end
  ...
  it 'returns the number of correct colors (1)' do
    code          = %w(R B G Y)
    guess         = 'RRRR'
    guess_checker = GuessChecker.new(code, guess)
    expect(guess_checker.color_only).to eq(1)
  end

  it 'returns the number of correct colors (2)' do
    code          = %w(R B G Y)
    guess         = 'RRBB'
    guess_checker = GuessChecker.new(code, guess)
    expect(guess_checker.color_only).to eq(2)
  end
